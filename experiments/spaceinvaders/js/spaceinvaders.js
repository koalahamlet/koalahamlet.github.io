...
PlayState.prototype.draw = function(game, dt, ctx) {
    ...
    //  Draw image
      ...
      //   ctx.drawImage(imageObj, this.ship.x - (this.ship.width / 2), this.ship.y - (this.ship.height / 2), this.ship.width, this.ship.height);
      // };
      var shipImageObj = new Image();
      shipImageObj.src = 'stipe.png';
      var shipPattern = ctx.createPattern(shipImageObj, "no-repeat");
      ctx.fillStyle = shipPattern;
  // context.fillRect(x, y, width, height);
    // ctx.fillStyle = '#ffffff';
    // ctx.fillRect(this.ship.x - (this.ship.width / 2), this.ship.y - (this.ship.height / 2), this.ship.width, this.ship.height);
    ctx.drawImage(shipImageObj, this.ship.x - (this.ship.width / 2), this.ship.y - (this.ship.height / 2), this.ship.width, this.ship.height);

    //  Draw invaders.
    // ctx.fillStyle = '#006600';
    var invaderImageObj = new Image();
      invaderImageObj.src = 'johnny.png';
      var invaderPattern = ctx.createPattern(invaderImageObj, "no-repeat");
      ctx.fillStyle = invaderPattern;
    for(var i=0; i<this.invaders.length; i++) {
        var invader = this.invaders[i];
        ctx.drawImage(invaderImageObj, invader.x - invader.width/2, invader.y - invader.height/2, invader.width, invader.height);
    }

    //  Draw bombs.
    var bombImageObj = new Image();
      bombImageObj.src = 'willis.png';
      var bombPattern = ctx.createPattern(bombImageObj, "no-repeat");
      ctx.fillStyle = bombPattern;
    ...
    //  Draw rockets.
    var rocketsImageObj = new Image();
      rocketsImageObj.src = 'moby.png';
      var rocketsPattern = ctx.createPattern(rocketsImageObj, "no-repeat");
      ctx.fillStyle = rocketsPattern;
